...
def train_mahalanobis_metric_for_fold(fold_train_set, fold_name, annotations_filepath = "all_annotations.json"):
    df = load_all_maps_into_df(annotations_filepath, fold_train_set)
    os.makedirs("cross_validation_mahalanobis_results/" + fold_name, exist_ok = True)
    df.to_csv("cross_validation_mahalanobis_results/" + fold_name + "/" + "train_pairs_data.csv", index=False)
    return find_mahalanobis_metric("cross_validation_mahalanobis_results/" + fold_name + "/" + "train_pairs_data.csv")
def predict_on_test_set(fold_name, fold_test_set, mahalanobis_metric, annotations_filepath = "all_annotations.json"):

    name = "mahalanobis_metric"
    print(name)
    linkage_method = LinkageMethod(prims_mst, MahalanobisMetric(mahalanobis_metric))
    map_list_compare_linkages(fold_test_set, name, annotations_filepath, linkage_method, "cross_validation_mahalanobis_results/" + fold_name + "/" + name + ".json")
    recall = get_stats_from_results_file("cross_validation_mahalanobis_results/" + fold_name + "/" + name + ".json", annotations_filepath)
...
if __name__ == "__main__":
    ...
    for fold in folds:
        ...
        os.makedirs("cross_validation_mahalanobis_results/" + fold_name, exist_ok = True)
        mahalanobis_metric = train_mahalanobis_metric_for_fold(fold["train"], fold_name)
        stats_line = predict_on_test_set(fold_name, fold["test"], mahalanobis_metric)
